...
struct alignment **align;
{
    ...
    for(j = 0; j <= ny; j++) {
        for(i = 0; i <= nx; i++) {
            ...
            dist(i-1, j-1) + (*dist_funct)(x[i-1], y[j-1], 0, 0)    : MAXINT;
            d2 = i>0 ? /* deletion */
            dist(i-1, j) + (*dist_funct)(x[i-1], 0, 0, 0)    : MAXINT;
            d3 = j>0 ? /* insertion */
            dist(i, j-1) + (*dist_funct)(0, y[j-1], 0, 0)    : MAXINT;
            d4 = i>1 && j>0 ? /* contraction */
            dist(i-2, j-1) + (*dist_funct)(x[i-2], y[j-1], x[i-1], 0)    : MAXINT;
            d5 = i>0 && j>1 ? /* expansion */
            dist(i-1, j-2) + (*dist_funct)(x[i-1], y[j-2], 0, y[j-1])    : MAXINT;
            d6 = i>1 && j>1 ? /* melding */
            dist(i-2, j-2) + (*dist_funct)(x[i-2], y[j-2], x[i-1], y[j-1])    : MAXINT;
            dmin = d1;
            if(d2<dmin) dmin=d2;
            if(d3<dmin) dmin=d3;
            if(d4<dmin) dmin=d4;
    ...
    *align = (struct alignment *) malloc(n * sizeof(struct alignment));
    for(i=0; i<n; i++){
        //bcopy(ralign + i, (*align) + (n-i-1), sizeof(struct alignment));              //for unix, bcopy alternative is memmove
        memmove((*align) + (n-i-1), ralign + i, sizeof(struct alignment));
    }
    free(distances);
    free(path_x);
    free(path_y);
...
char **argv;
{
    ...
    hard_end1 = hard_regions1 + number_of_hard_regions1;printf(" hard1 : %d",number_of_hard_regions1);
    hard_end2 = hard_regions2 + number_of_hard_regions2;printf(" hard2 : %d",number_of_hard_regions2);
...
int *len_ptr;
{
    ...
    *len_ptr = stat_buf.st_size;
    //*len_ptr = stat_buf.st_size - 1;          //enter=\r\n instead of just \n, so size becomes +1 for each enter --VBL
    result = malloc(*len_ptr);
    if(result == NULL)
        err("malloc failed\n");
    read_size = fread(result, sizeof(char), *len_ptr, fd);

    if(read_size != *len_ptr)
        err("fread failed");
...
int *len_ptr;
{
    char *s, **result;
    int i = 0;
    while(string < end && *string == delimiter)
        string++;
